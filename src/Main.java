...
public class Main {
    public static void main(String[] args) {
        ...
        long minorAge = persons.stream().filter(person -> person.getAge() < 18).count();
        System.out.println("Несовершеннолетних" +minorAge);

        List<String> conscript = persons.stream().filter(person -> (person.getAge() >= 18 && person.getAge() <= 27))
                .filter(person -> person.getSex() == Sex.MAN).map(Person::getFamily)
                .collect(Collectors.toList()); {
        for (String c : conscript)
            System.out.println("Призывник: "+ c);}
